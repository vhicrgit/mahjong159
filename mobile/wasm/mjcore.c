...
typedef struct {
    ...
    i8 u0[NTILE];
    double rho;
    int kaizen, kai_margin, kai_max;
} HvCtx;
static HvCtx hv = { {0}, {0}, 1.0, 0, 2, 2 };
...
/* rho/kaizen 变了会让旧条目失效(键里不含这两个参数), 所以切换时清表 */
...
static double E_rec(const i8 *hand, const i8 *u, int kai) {
    ...
    int ut[NTILE], uw[NTILE], nu = 0;
    u32 um = useful_mask_of(hand);
    for (int t = 0; t < NTILE; t++)
        if (((um >> t) & 1u) && u[t] > 0) { ut[nu] = t; uw[nu] = u[t]; nu++; }

    /* 碰通道: 可碰且碰后最优向听严格降低的对子, t 升序 */
    int pt[27], pd[27], np = 0;
    double pw[27];
    if (hv.rho > 0) {
        i8 h2[NTILE];
        for (int t = 0; t < 27; t++) {
            ...
            if (bs < s) { pt[np] = t; pw[np] = (double)u[t] * 3.0 * hv.rho; pd[np] = bd; np++; }
        }
    }

    /* 换型通道(kaizen): 不降向听但让有效张变宽 >= kai_margin。
     * kh 必须是局部数组 —— E_rec 是递归的, 用 static 会被子调用覆盖。
     * (Bot 对战走 kaizen=false 碰不到这里, 但分析面板可能开 kaizen) */
    int kt[NTILE], kw[NTILE], nk = 0;
    i8 kh[NTILE][NTILE];
    if (hv.kaizen && kai < hv.kai_max) {
        ...
        for (int t = 0; t < NTILE; t++) {
            ...
            if (ukeire_of(h14, u) >= uk0 + hv.kai_margin) {
                kt[nk] = t; kw[nk] = u[t];
                for (int i = 0; i < NTILE; i++) kh[nk][i] = h14[i];
                nk++;
            }
        }
    }
    ...
    for (int i = 0; i < nu; i++) sU += (double)uw[i];
    for (int i = 0; i < nk; i++) sK += (double)kw[i];
    for (int i = 0; i < np; i++) sP += pw[i];
    double U = sU + sK + sP;

    double val;
    if (U <= 0) {
        val = (double)N + 2.0 * (double)s;      /* 有效张耗尽的死手 */
    } else {
        val = ((double)N + 1.0) / (U + 1.0);    /* 无放回首次命中的精确期望 */
        i8 h[NTILE], u2[NTILE];
        for (int i = 0; i < nu; i++) {           /* 自摸通道 */
            double p = (double)uw[i] / U;
            int t = ut[i];
        ...
        for (int i = 0; i < nk; i++) {           /* 换型通道 */
            double p = (double)kw[i] / U;
            for (int j = 0; j < NTILE; j++) u2[j] = u[j];
            u2[kt[i]]--;
            val += p * E_rec(kh[i], u2, kai + 1);
        }
        for (int i = 0; i < np; i++) {           /* 碰通道 */
            double p = pw[i] / U;
            int t = pt[i];
            for (int j = 0; j < NTILE; j++) h[j] = hand[j];
            h[t] -= 2;
...
void mj_hv_set(double rho, int kaizen) {
    if (hv.rho != rho || hv.kaizen != kaizen) e_memo_clear();
    hv.rho = rho;
    hv.kaizen = kaizen;
