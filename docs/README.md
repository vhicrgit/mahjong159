...
**棋力格局**（CRN 配对、轮坐四座位、名次奖励差 分/局，vs v31n 规则 bot）:

| 选手 | 首胡(线上规则) | 血战到底(仅训练用) |
|---|---|---|
| **bc_k0_r2（上线 NN）** | **-0.030 ± 0.033（打平）** | **+0.012 ± 0.043（打平）** |
| 牌型分析器本体（学者, kai=0/1） | -0.075±0.061 / -0.110±0.083 | -0.061±0.078 |
| 旧 RL 产物 acnn_v2（已备份） | -0.238 ± 0.042 | -0.573 ± 0.057 |
| bc_k0_r2 vs acnn_v2（h2h） | **+0.169 ± 0.036 (t=4.7)** | **+0.438 ± 0.051 (t=8.7)** |

即：**NN、分析器、v31n 三者统计打平**，构成平台期。NN 的价值 = 分析器级
棋力 + 快约 3 个数量级的推理（毫秒级前向 vs 整表 E 递推）。
