...
最后更新: 2026-08-26（深夜，判胡/向听核心4个bug修复+重基线，见 ceiling_and_bots.md 第十二节）

> **⚠ 2026-08-26 深夜重要修正**: rules/win.py 判胡/向听发现4个长期bug
> （红中补低位顺子漏胡2.7%、8/9点搭子不计、DFS贪心丢最优、对子不当搭子），
> 已全部修复并通过 8.5万手一致性验证; 向听 DFS 增加 need 参数原生支持副露手
> （虚拟刻子填充法已废弃）。**下表所有历史绝对数字在有bug的规则下测得**，
> 仅供相对参考; 修复后新基线: v1=25.6%±1.6%, **v10=33.0%±1.7%**（v10 的碰杠
> 规则随 shanten 根修自动变正确, v31≡v10 已归档）。NN 需用新规则重训。
