...
class Analyzer:
    ...
    def analyze_discards(self) -> list[dict]:
        ...
        for t in range(TILE_COUNT):
            ...
            # (向听高 -> 牌型散 -> 能帮上忙的牌种多)。跳档相加就是拿两个不同
            # 尺度的量做加法。实测: 1条 4条5条6条6条 3饼4饼6饼7饼 4万5万5万5万8万(向听2)
            #   打3饼 -> 向听3, 进张86 -> 旧分 -42   (排第1)
            #   打1条 -> 向听2, 进张36 -> 旧分 -92
            # 退一档向听扣 100, 但多 50 张进张加 150, 净赚 50 —— 公式判定
            # "主动退向听"划算, 所有退向听的牌都排到了保持向听的牌前面。
            # (原公式里的 -30*放杠风险 曾偶然遮盖了这一点; 风险项去掉后裸露。)
            #
            # 现在把 score 编码成严格字典序: 档位权重 10000 远大于 ukeire 的
            # 上界(28 种牌 x 4 张 = 112), 所以向听永远压倒进张, 永不会推荐退向听。
            # 同向听同进张时依赖稳定排序保持 tile 升序(与 JS 一致)。
            # score 现在是整数, 不再需要 _r() 抹平银行家舍入与 JS 的差异。
            # gang_risk 仍然计算并输出, 供分析面板展示参考, 不进排序。
            score = -10000 * s + ukeire
            out.append({
                "tile": t,
                "name": tile_short(t),
                "shanten": s,
